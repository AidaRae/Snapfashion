...
<div class="p-4 sm:p-6 lg:p-8 pb-20 lg:pb-8">
    ...
    <div class="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500 mb-5">
        ...
        <svg width="12" height="12" fill="none" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
        <span class="text-gray-600 dark:text-gray-300 font-medium">Add Product</span>
    </div>
    ...
    <div class="s-card">
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="p-5 sm:p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="form-input" placeholder="e.g. Vintage Denim Jacket" required>
                        @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="form-label">Description</label>
                        <textarea name="description" rows="5" class="form-input" placeholder="Type a detailed description of the product...">{{ old('description') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="sale_price" value="{{ old('sale_price') }}" class="form-input" placeholder="0.00">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" name="unit" value="{{ old('unit') }}" class="form-input" placeholder="e.g. pc, kg, pack">
                            @error('unit')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">Min. Stock Warning</label>
                            <input type="number" name="low_stock_qty" value="{{ old('low_stock_qty', 0) }}" class="form-input" placeholder="0" min="0">
                            @error('low_stock_qty')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">Max. Purchase Qty</label>
                            <input type="number" name="max_purchase_qty" value="{{ old('max_purchase_qty') }}" class="form-input" placeholder="Unlimited" min="1">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="sizes" value="{{ old('sizes') }}" class="form-input" placeholder="e.g. S, M, L, XL">
                            @error('sizes')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">Colors (comma-separated)</label>
                            <input type="text" name="colors" value="{{ old('colors') }}" class="form-input" placeholder="e.g. Black, White, Red">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="sku" value="{{ old('sku') }}" class="form-input" placeholder="PROD-001">
                    ...
                    <div>
                        ...
                        <label class="file-drop block relative">
                            <input type="file" name="image" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*" onchange="previewImage(this, 'imagePreview', 'imagePreviewContainer', 'fileName')">
                            ...
                            <span id="fileName" class="text-sm text-gray-500 font-medium">Click to upload image</span>
                        </label>
                    ...
                    <div>
                        ...
                        <label class="file-drop block relative">
                            <input type="file" name="hover_image" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*" onchange="previewImage(this, 'hoverPreview', 'hoverPreviewContainer', 'hoverFileName')">
                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" class="mx-auto mb-2 text-gray-400">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 15v3a2 2 0 002 2h14a2 2 0 002-2v-3M12 15V3m0 0L8.5 6.5M12 3l3.5 3.5"/>
                            </svg>
                            <span id="hoverFileName" class="text-sm text-gray-500 font-medium">Click to upload hover image</span>
                        </label>
                    ...
                    <div>
                        ...
                        <label class="file-drop block relative">
                            ...
                            <span id="galleryFileName" class="text-sm text-gray-500 font-medium">Click to upload gallery images</span>
                        </label>
                    ...
                    <div class="space-y-3 pt-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" class="w-4 h-4 text-brand rounded focus:ring-brand" {{ old('is_active', true) ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Status: Active</span>
                        </label>
                        
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="featured" value="1" class="w-4 h-4 text-brand rounded focus:ring-brand" {{ old('featured') ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Feature on Homepage</span>
                        </label>

                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_purchasable" value="1" class="w-4 h-4 text-brand rounded focus:ring-brand" {{ old('is_purchasable', true) ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Can Purchasable</span>
                        </label>

                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="show_stock_out" value="1" class="w-4 h-4 text-brand rounded focus:ring-brand" {{ old('show_stock_out', true) ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Show Stock Out</span>
                        </label>

                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_refundable" value="1" class="w-4 h-4 text-brand rounded focus:ring-brand" {{ old('is_refundable') ? 'checked' : '' }}>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 dark:border-neutral-800 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="btn-primary-custom">Save Product</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function previewImage(input, previewId, containerId, fileNameId) {
        if (input.files && input.files[0]) {
            ...
            document.getElementById(fileNameId).innerText = 'Click to upload image';
    ...
    function previewGallery(input) {
        ...
        if (input.files && input.files.length > 0) {
            label.innerText = input.files.length + ' image(s) selected';
            ...
            label.innerText = 'Click to upload gallery images';
        }
    }
</script>
@endpush
