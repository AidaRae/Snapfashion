...
@section('admin')
    ...
    <div class="p-4 sm:p-6 lg:p-8 pb-20 lg:pb-8">
        ...
        <div class="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500 mb-5">
            ...
            <span class="text-gray-600 dark:text-gray-300 font-medium">Add Category</span>
        </div>

        <div class="s-card max-w-3xl mx-auto">
            <div class="p-5 sm:p-8 border-b border-gray-100 dark:border-neutral-800">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">Add New Category</h2>
                <p class="text-sm text-gray-500 mt-1">Organize your products properly by creating categories.</p>
            </div>
            ...
            <form action="{{ route('admin.category.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <select name="parent_id" class="form-input">
                        ...
                        @php
                            $rootCategories = App\Models\Category::where('parent_id', 0)->orderBy('sort', 'asc')->orderBy('title', 'asc')->get();
                        @endphp
                        @foreach ($rootCategories as $rootCat)
                            <option value="{{ $rootCat->id }}" {{ old('parent_id', request('parent')) == $rootCat->id ? 'selected' : '' }}>
                                {{ $rootCat->title ?? $rootCat->name }}
                            </option>
                            @php
                                $subCategories = App\Models\Category::where('parent_id', $rootCat->id)->orderBy('sort', 'asc')->orderBy('title', 'asc')->get();
                            @endphp
                            @foreach ($subCategories as $subCat)
                                <option value="{{ $subCat->id }}" {{ old('parent_id', request('parent')) == $subCat->id ? 'selected' : '' }}>
                                    &nbsp;&nbsp;&nbsp;-- {{ $subCat->title ?? $subCat->name }}
                                </option>
                                @php
                                    $subSubCategories = App\Models\Category::where('parent_id', $subCat->id)->orderBy('sort', 'asc')->orderBy('title', 'asc')->get();
                                @endphp
                                @foreach ($subSubCategories as $subSubCat)
                                    <option value="{{ $subSubCat->id }}" {{ old('parent_id', request('parent')) == $subSubCat->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="form-input" placeholder="A short description...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="img-uploader" id="uploaderThumb">
                            ...
                            <div class="preview-state">
                                <div class="preview-img-wrap">
                                    <img src="" alt="Preview" class="preview-img" id="previewThumb">
                                </div>
                                <div class="preview-bar">
                                    ...
                                    <div class="preview-meta">
                                        <div class="preview-name" id="nameThumb">—</div>
                                        <div class="preview-size" id="sizeThumb">—</div>
                                    </div>
                    ...
                    <div>
                        ...
                        <div class="img-uploader" id="uploaderIcon">
                            ...
                            <div class="preview-state">
                                <div class="preview-img-wrap">
                                    <img src="" alt="Preview" class="preview-img" id="previewIcon">
                                </div>
                                <div class="preview-bar">
                                    <div class="preview-file-icon">
                                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
                                    ...
                                    <div class="preview-meta">
                                        <div class="preview-name" id="nameIcon">—</div>
                                        <div class="preview-size" id="sizeIcon">—</div>
                                    </div>
                                    <div class="preview-actions">
                                        <button type="button" class="preview-btn" title="Replace"
